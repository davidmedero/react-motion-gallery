...
export const componentCatalog: ComponentSurface[] = [
  ...
  },
  {
    ...
    categoryIds: ["grid"],
    description:
      "Responsive media grid with spans, template columns, skeletons, fullscreen item handoffs, and child-list data plugins.",
    whenToUse: [
  ...
  },
  {
    ...
    categoryIds: ["masonry"],
    description:
      "Lightweight dimensioned masonry layout for image-first grids, with opt-in fullscreen, data plugins, and measured subpaths for arbitrary card heights.",
    whenToUse: [
  ...
  },
  {
    ...
    categoryIds: ["entries"],
    description:
      "Structured content renderer for rows with section text, body copy, media, metadata, slider/grid/masonry media layouts, data-window plugins, and readiness.",
    whenToUse: [
  ...
  },
  {
    ...
    description:
      "Small accessible rating display primitive for product cards, reviews, and commerce metadata.",
    whenToUse: [
...
const categoryDescriptions: Record<DemoCategoryId, string> = {
  ...
  grid:
    "Responsive grid demos for spans, template tracks, min column width, lazy-load plugins, data plugins, video, fullscreen, and skeletons.",
  masonry:
    "Masonry demos for the fullscreen-enabled light image core plus measured text-wrap, data plugins, video, fullscreen, and stable skeleton loading.",
  entries:
    "Structured entry demos that combine copy, metadata, media, slider/grid/masonry renderers, API-backed data controls, and readiness.",
  "zoom-pan":
