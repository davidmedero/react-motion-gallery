...
export type GridReadyController = {
  ref: React.RefCallback<GridHandle>;
  ready: boolean;
  handleRef: React.MutableRefObject<GridHandle | null>;
};

export function useGridReady(): GridReadyController {
  const handleRef = React.useRef<GridHandle | null>(null);
  ...
  const ref = React.useCallback(
    (handle: GridHandle | null) => {
      ...
      const sameGrid = !!handle && !!previousRoot && previousRoot === nextRoot;
