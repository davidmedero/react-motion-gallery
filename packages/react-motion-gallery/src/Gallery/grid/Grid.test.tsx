...
describe("Grid item spans and template columns", () => {
  ...
  test("exposes Grid.Item and applies item host props for cloned and wrapped children", () => {
    const markup = renderToStaticMarkup(
      <Grid columns={12} loading={{ enabled: false }}>
  ...
  test("ignores spans in auto-fill minColumnWidth mode and warns once", () => {
    ...
    const markup = renderToStaticMarkup(
      <Grid minColumnWidth={220} loading={{ enabled: false }}>
  ...
  test("applies the same spans to the default loading skeleton", () => {
    const markup = renderToStaticMarkup(
      <Grid columns={12}>
        <Grid.Item span={6}>alpha</Grid.Item>
  ...
  test("keeps span styles when loading skeleton uses slot overrides", () => {
    const markup = renderToStaticMarkup(
      <Grid
        columns={12}
        loading={{
  ...
  test("keeps span styles when passthrough mode wraps each child", () => {
    const markup = renderToStaticMarkup(
      <Grid columns={12} renderMode="passthrough" loading={{ enabled: false }}>
        <Grid.Item span={6}>
