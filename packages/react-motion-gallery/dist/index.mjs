...
function normalizeItemsInput(v) {
    ...
    if (shouldMountContent) {
      ...
      const mediaNodes = mediaArray.map((media, mediaIndex) => {
        ...
        return /* @__PURE__ */ jsx(
          ...
          {
            type: "button",
            className: styles_default.entryMediaButton,
            onClick: handleClick,
            ref: reg,
            children: rawContent
          },
...
var isStringArray2 = (v) => Array.isArray(v) && v.every((x) => typeof x === "string");
var normalizeFsItems = (v) => {
  if (!v || !v.length) return [];
  return isStringArray2(v) ? toMediaItems(v) : v;
};
function nodeFromMediaDefault(m) {
  if (m.kind === "image") return /* @__PURE__ */ jsx("img", { src: m.src, alt: m.alt ?? "" });
  if (m.kind === "video") return /* @__PURE__ */ jsx("video", { src: m.src, controls: true, preload: "metadata" });
  return null;
}
...
function Entries(props) {
  ...
  React24.useEffect(() => {
    ...
    core.registerFullscreenAdapter("entries", {
      ...
      getOwnerSliderHandle: (globalIndex) => {
  ...
  const openFullscreenAt = React24.useCallback(
    (globalIndex, originEl) => {
      ...
      if (core?.requestFullscreenOpen) {
        core.requestFullscreenOpen({ source: "entries", index: globalIndex, img: imgEl, event: void 0 });
        return;
      }
...
var FullscreenModal = ({
  ...
  async function proceedToClose() {
    ...
    const slot = expandableImgRefs.current[canonicalIdx2] ?? null;
    console.log("slot", slot);
    console.log("expandableImgRefs.current", expandableImgRefs.current);
    const slotCurrent = slot && typeof slot === "object" && "current" in slot ? slot.current : slot;
    let destImg = null;
    if (slotCurrent?.tagName === "IMG") {
      destImg = slotCurrent;
    } else if (slotCurrent) {
      const host = slotCurrent;
      destImg = host.querySelector?.("img");
    }
    if (!destImg) {
      const host = document.querySelector(`[data-rmg-idx="${canonicalIdx2}"]`);
      destImg = host?.querySelector("img") ?? null;
    }
    if (!destImg) {
      safeTeardown();
      return;
...
function ThumbnailSlider({
  ...
  useEffect(() => {
    ...
    const unsub = ch.subscribe(() => {
      ...
      selectedIndexRef.current = clamp2(index, 0, Math.max(0, count - 1));
  ...
  useEffect(() => {
    ...
    const init = clamp2(channelRef.current.get().index ?? 0, 0, maxIndex);
    setActiveThumb(init);
  ...
  function getCenteredScroll(i) {
    ...
    const maxScroll = Math.max(0, contentLength - view);
    return clamp2(centerWanted, 0, maxScroll);
  }
  function getStartSnapScroll(i) {
    ...
    const view = lay.cw || containerLength || 0;
    const maxScroll = Math.max(0, contentLength - view);
    return clamp2(o.start, 0, maxScroll);
  }
  ...
  useEffect(() => {
    ...
    const i = clamp2(channelRef.current.get().index ?? 0, 0, Math.max(0, count - 1));
...
function usePanDrag(d) {
  ...
  const handlePanPointerStart = React24.useCallback(
    (e, imageRef) => {
      ...
      trackerRef.current = DragTracker3(axis, ownerWin);
                ...
                "div",
                {
                  ...
                  children: /* @__PURE__ */ jsx(
                    FullscreenSlider,
                    {
                      ...
                      handleZoomToggle: (e, imageRef) => handleZoomToggle2(zoomCtx, e, imageRef),
...
function useFullscreenController(args) {
  ...
  const adapterFor = useCallback(
    (source) => getFullscreenAdapter(source),
    [getFullscreenAdapter]
  );
  ...
  const fallbackEntryMapRef = useRef(null);
  ...
  const safeEntryMediaLayout = entryCtx.entryMediaLayout ?? safeEntriesObject.mediaLayout ?? "slider";
  const canMountEntryOverlay = layout === "entries" && !!entryCtx.entriesObject && // <-- critical
  ...
  const getOwnerSliderHandle = useCallback(
    (index) => {
      if (layout === "entries" && safeEntryMediaLayout === "slider") {
        ...
        const entryHandle = link && entrySliderRefs ? entrySliderRefs[link.entryIndex] : null;
  ...
  const requestFsCloseRef = useRef(null);
  const fs = useMemo(() => {
    return {
      ...DEFAULT_FULLSCREEN,
      ...fullscreen ?? {},
  ...
  function resolveFsCaptionPlacement(placement, breakpoint, viewportWidth2) {
    ...
    if (breakpoint != null && viewportWidth2 < breakpoint) return "bottom";
    return placement;
  }
  useEffect(() => {
    if (typeof window === "undefined") return;
  ...
  const fullscreenNode = fs.enabled ? /* @__PURE__ */ jsx(
    FullscreenRuntime_default,
    {
...
function DragTracker4(main, ownerWindow) {
  const scroll = main ?? "x";
...
var Slider = forwardRef(function Slider2({
  ...
  useEffect(() => {
    ...
    const dragStore = EventStore();
    const moveStore = EventStore();
    const tracker = DragTracker4(AX.main, window);
    let isMouse = false;
    let startMain = 0;
    let startCross = 0;
    let preventScroll = false;
...
function buildScopedSkeletonCountCss(args) {
  ...
  const clamp3 = (n) => Math.max(0, Math.min(maxSlots, Math.floor(n)));
  const baseCount = clamp3(rules[0]?.count ?? fallbackCount);
...
var SliderLayout = React24.forwardRef(function SliderLayout2(props, forwardedRef) {
  ...
  const shimmerStyleVars = React24.useMemo(() => {
    ...
    return {
      ...s.radius != null ? { ["--rmg-shimmer-radius"]: px2(s.radius) } : {},
...
function GridLayout({
  ...
  const shimmerStyleVars = React24.useMemo(() => {
    ...
    const px2 = (v) => v == null ? void 0 : typeof v === "number" ? `${v}px` : v;
    return {
      ...s.paddingBottom != null ? { ["--rmg-shimmer-padding-bottom"]: px2(s.paddingBottom) } : {},
      ...s.radius != null ? { ["--rmg-shimmer-radius"]: px2(s.radius) } : {},
      ...s.c1 != null ? { ["--rmg-shimmer-c1"]: s.c1 } : {},
      ...s.c2 != null ? { ["--rmg-shimmer-c2"]: s.c2 } : {},
      ...s.c3 != null ? { ["--rmg-shimmer-c3"]: s.c3 } : {},
  ...
  const gridChildren = React24.useMemo(() => {
    return cells.map((cell, index) => {
      ...
      const baseClassName = [
        gridItemBaseClass,
        styles_default.gridItem,
        styles_default.introItem,
        grid.itemClassName || ""
      ].filter(Boolean).join(" ");
  ...
  const containerProps = {
    className: [
      styles_default.gridRoot,
      styles_default.introContainer,
...
function pickImgEl(node) {
  if (!node) return null;
  if (node.tagName === "IMG") return node;
  return node.querySelector("img");
}
function GridLayoutRuntime(props) {
  ...
  const gridObject = React24.useMemo(() => {
    ...
    const fallbackGap = DEFAULT_GRID.gap != null ? DEFAULT_GRID.gap : 0;
    const resolvedGap = gridOptions.gap != null ? Math.max(
      0,
      resolveNumberFromResponsive(
        gridOptions.gap,
        fallbackGap,
    ...
    return {
      ...
      minColumnWidth: gridOptions.minColumnWidth ?? DEFAULT_GRID.minColumnWidth,
      gap: resolvedGap
    };
...
var DefaultMasonrySkeleton = ({
  ...
  const MIN_RATIO = 25;
  const MAX_RATIO = 220;
  const safeRatios = Array.isArray(ratios) && ratios.length ? ratios.map((r) => Number(r)).filter((r) => Number.isFinite(r)).map((r) => Math.max(MIN_RATIO, Math.min(MAX_RATIO, r))) : DEFAULT_RATIOS;
...
function MasonryLayout({
  ...
  const defaultMasonrySkeleton = /* @__PURE__ */ jsx("div", { className: styles_default.gridSkeletonOverlay, children: /* @__PURE__ */ jsx(
    DefaultMasonrySkeleton,
    {
      ...
      gapPx: masonryGapPx,
      classNames: {
        root: styles_default.gridSkeletonMasonryRoot,
        column: styles_default.gridSkeletonMasonryCol,
        item: styles_default.gridSkeletonItem
      },
    ...
    /* @__PURE__ */ jsx(
      Masonry,
      {
        ...
        masonryClassNames: {
          ...
          column: [styles_default.masonryCol, masonry.classNames?.column].filter(Boolean).join(" "),
          item: [styles_default.masonryItem, masonry.classNames?.item].filter(Boolean).join(" ")
        },
        masonryStyle: {
          ["--rmg-intro-stagger"]: `${intro.staggerMs}ms`,
          ["--rmg-intro-transform"]: intro.transform,
...
function MasonryLayoutRuntime(props) {
  ...
  const masonryObject = React24.useMemo(() => {
    ...
    return {
      ...DEFAULT_MASONRY,
      ...src,
      placement: src?.placement ?? DEFAULT_MASONRY.placement,
      classNames: {
        ...DEFAULT_MASONRY.classNames,
        ...src?.classNames ?? {}
      },
      layout: {
        ...DEFAULT_MASONRY.layout,
        ...src?.layout ?? {}
      },
      transitions: {
        ...DEFAULT_MASONRY.transitions,
        ...src?.transitions ?? {}
      }
    };
  }, [masonryOptions]);
  ...
  const openFullscreenAt = React24.useCallback(
    (index, originEl) => {
      ...
      if (!imgEl) {
        const slot = expandableImgRefs.current[index] ?? null;
        const slotCurrent = slot && typeof slot === "object" && "current" in slot ? slot.current : slot;
        if (slotCurrent?.tagName === "IMG") imgEl = slotCurrent;
        else if (slotCurrent) imgEl = slotCurrent.querySelector?.("img") ?? null;
      }
      isClick.current = true;
      core.requestFullscreenOpen({
        source: "masonry",
  ...
  const masonryChildren = React24.useMemo(() => {
    return buildMasonryChildren({
      ...
      openFullscreenAt: (i, originEl) => openFullscreenAt(i, originEl),
      registerExpandableImg: (i, node) => registerExpandableImg(i, node),
