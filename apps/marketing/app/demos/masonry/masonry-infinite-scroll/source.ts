...
import styles from "./masonry-infinite-scroll-demo.module.css";
...
export function MasonryInfiniteScrollDemo() {
  ...
  const displayProducts = useMemo(() => {
    ...
    const slottedProducts = useInitialSlots
      ? products.map((product, index) =>
          index < PAGE_SIZE
            ? {
                ...product,
  ...
  const plugins = useMemo(
    ...
    [infinitePlugin, products.length, total, virtualizationPlugin],
  );
