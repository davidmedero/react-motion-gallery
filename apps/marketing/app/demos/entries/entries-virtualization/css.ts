...
.controls:has(.virtualizationMeter) {
  position: sticky;
  top: calc(var(--demos-sidebar-offset, 92px));
  z-index: 12;
  align-self: stretch;
}
...
.virtualizationMeter {
  ...
  box-shadow:
    0 12px 28px rgba(23, 33, 38, 0.08),
    0 1px 0 rgba(255, 255, 255, 0.88) inset;
  backdrop-filter: blur(14px);
}

.controls:has(.virtualizationMeter) .virtualizationMeter {
  position: relative;
  top: auto;
  z-index: auto;
}

.virtualizationMeterPrimary,
.virtualizationMeterSecondary {
  min-width: 0;
...
.skeletonTitle {
  width: var(--product-skeleton-title-width);
...
.skeletonBodyLine {
  width: var(--product-skeleton-line-width);
  height: 17px;
  border-radius: 999px;
}

@container product-skeleton-copy (min-width: 420px) {
  .skeletonBodyLine:nth-child(n + 4) {
    display: none;
  }
}
...
.skeletonInventoryLabel {
  width: 48px;
  height: 10px;
  border-radius: 4px;
}

.skeletonInventoryValue {
  width: 62px;
  height: 14px;
  border-radius: 5px;
}
