...
function fetchAllEntriesProductPage(signal?: AbortSignal) {
  return fetchEntriesProductPage({ limit: 0, skip: 0, signal });
}
...
export function EntriesPaginationGridClientDemo() {
  ...
  useEffect(() => {
    ...
    fetchAllEntriesProductPage(ac.signal)
      .then((page) => {
